...
CBOW uses a full softmax with cross-entropy loss and is optimized for small corpora. Training produces the input embedding matrix $W_{in}$, stored in model.embeddings, which can be used for similarity, analogy, or ConEc computations.
