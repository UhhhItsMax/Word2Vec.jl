using Test
using SparseArrays: nnz
using Word2Vec

@testset "ContextMatrix" begin
    path = joinpath(@__DIR__, "data", "context_matrix.txt")

    @testset "co-occurrence counts" begin
        token_to_id = Dict("a" => 1, "b" => 2, "c" => 3, "d" => 4)
        coocs = Word2Vec.sparse_context_matrix.get_co_occurence_counts(path, token_to_id, 1)

        @test coocs[(1, 2)] == 1
        @test coocs[(3, 2)] == 1
        @test coocs[(2, 3)] == 1
        @test coocs[(4, 3)] == 1
    end

    @testset "build matrix" begin
        cm = Word2Vec.sparse_context_matrix.build_context_matrix_from_file(path; window_size=1, min_count=1)
        @test size(cm.mat) == (4, 4)
        @test nnz(cm.mat) == 4
        @test length(cm.vocab) == 4
        @test length(cm.token_to_id) == 4
    end
end
