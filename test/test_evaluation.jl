...
@testset "cosine_similarity" begin
    ...
    v1 = [1.0, 0.0]
    v2 = [0.0, 1.0]
    @test isapprox(cosine_similarity(v1, v2), 0.0; atol = 1.0e-8)

    # Test 2: same vector
    v = [1.0, 2.0, 3.0]
    @test isapprox(cosine_similarity(v, v), 1.0; atol = 1.0e-8)

    # Test 3: opposite vector
    u = [-1.0, -2.0, -3.0]
    @test isapprox(cosine_similarity(v, u), -1.0; atol = 1.0e-8)

    # Test 4: arbitrary vectors
    a = [1.0, 2.0]
    b = [2.0, 3.0]
    expected = dot(a, b) / (norm(a) * norm(b))
    @test isapprox(cosine_similarity(a, b), expected; atol = 1.0e-8)
